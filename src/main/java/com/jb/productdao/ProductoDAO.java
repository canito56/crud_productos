...
import com.jb.productconnection.ProductConnection;
...
public class ProductoDAO {
	...
	public boolean editar(Producto producto) throws SQLException {
		...
		try {
			...
			sql = "UPDATE productos " +
				  "set nombre = ?, cantidad = ?, precio = ?, fecha_actualizar = ?" + 
				  "WHERE id = ?";
			statement = connection.prepareStatement(sql);
	...
	public List<Producto> obtenerProductos() throws SQLException {
		...
		try {
			...
			statement = connection.prepareStatement(sql);
			System.out.println("sentencia select: " + statement);
	...
	public Producto obtenerProducto(int idProducto) throws SQLException {
		...
		try {
			...
			statement = connection.prepareStatement(sql);
			statement.setInt(1, idProducto);
			System.out.println("sentencia select where: " + statement);
			resultSet = statement.executeQuery(); 
			if (resultSet.next()) {
				p.setId(resultSet.getInt(1));
				p.setNombre(resultSet.getString(2));
	...
	private Connection obtenerConexion() throws SQLException {
		return ProductConnection.getConnection();
	}
